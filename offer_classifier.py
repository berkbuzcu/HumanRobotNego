...
class OfferClassifier:
	...
	def get_offer_and_arguments(self, input):
		...
		if is_acceptance:
			return self.action_factory.create_acceptance()
			...
			if ((sentence_subject == "I" and not any(word for word, tag in sentence if word.lower() in ["n't", "not", "dont"]) and sentence_verb.lower() in ["have", "take", "get", "want", "like", "need", "keep"]) or
				...
				for (noun, index) in sentence_nouns:
					...
					if len(sentence) > 3 and (sentence[index-2][0].lower() == 'except'):
						...
						try:
							if len(keyword_match) > 0 and (sentence[index-1][1] == 'NUM' and sentence[index-1][0] in ['0', '1', '2', '3', '4']):
								#print(keyword_match[0], int(sentence[index-1][0]))
...
if __name__ == "__main__":
	human_utility_space = UtilitySpace("D:\PythonProjects\Human_Robot_Nego\HANT\Domains\Holiday_A\Berk\Human.xml")
	action_factory = ResourceAllocationActionFactory(human_utility_space, "Human")

	print("table index: ", human_utility_space.issue_value_evaluation)
	
	print([values for values in human_utility_space.issue_values_list.values()])

	indices = {}
	for issue_idx, (issue, values) in enumerate(human_utility_space.issue_value_evaluation.items()):
		for value_idx, item in enumerate(values):
			indices[item] = (issue_idx, value_idx)

	print(indices)

	#classifier = OfferClassifier(human_utility_space, action_factory)
	#offer = classifier.get_offer_and_arguments("I want 4 apples")
	#print(offer[0].get_bid())