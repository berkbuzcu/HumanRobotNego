...
class HolidayOfferClassifier:
	...
	def preprocess_input(self, input):
		input = input.lower()
		issue_values_list = domain.issue_values_list
		issues_list = [[x] * len(issue_values_list[x]) for x in issue_values_list.keys()]
		value_issue_dict = dict(ChainMap(*[dict(zip(*i)) for i in zip(issue_values_list.values(), issues_list)]))
		keywords = value_issue_dict.keys()
