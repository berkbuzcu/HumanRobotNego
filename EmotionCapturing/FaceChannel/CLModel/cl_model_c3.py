...
import numpy
from CLModel.GDM_Imagine_Dimensional.episodic_gwr import EpisodicGWR
from CLModel.GDM_Imagine_Dimensional import gtls
from CLModel.FaceChannelUtils import modelDictionary, modelLoader
...
CAAE_LoadPath = "CLModel/checkpoint/"
...
def generate_images(loadPath, valence, arousal, sess, graph, path_to_dir='./logs/faces/', path_to_out_dir='./logs/faces_imagined/'):
...
def generate_encodings_facechannel(path_to_dir, sess, graph, mode='imagine'):
    '''
        dataX: Imagine,
...
def apply_network_to_images_of_dir(loadPath, sess, graph, path_to_dir='./logs/faces/', path_to_out_dir='./logs/faces_imagined/'):
    """
    ...
    generate_images(loadPath, valence, arousal, sess, graph, path_to_dir, path_to_out_dir)

    return generate_encodings_facechannel(path_to_dir=path_to_out_dir, mode='imagine', sess=sess, graph=graph)
...
def get_arousal_valence(log_dir: str, graph, sess, use_imagined: bool = True):
    output_trained_GWRs = None

    if not os.path.isdir(log_dir + 'imagined'):
        os.mkdir(log_dir + 'imagined')

    gwrs_path = log_dir + "/saved_GWRs/"
    ...
    if len(os.listdir(gwrs_path)) > 0:
        ...
        # os.mkdir(gwrs_path + '/GDM_E')
        # os.mkdir(gwrs_path + '/GDM_S')
    ...
    # Imagining Faces
    if use_imagined:
        imagined_data, imagined_labels = apply_network_to_images_of_dir(loadPath=CAAE_LoadPath, sess=sess, graph=graph,
                                                                        path_to_dir=log_dir + 'faces/',
                                                                        path_to_out_dir=log_dir + 'imagined/')
