...
class EstimatedSensitivityCalculator:
    ...
    def get_opponent_moves_list(self, target_preference_profile, opponent_preference_profile, target_history):
        ...
        for i in range(len(target_history) - 1):
            ...
            if abs(target_bid_utility_list[i + 1] - target_bid_utility_list[i]) == 0 and abs(
                    human_bid_utility_list[i + 1] - human_bid_utility_list[i]) == 0:
                target_move_list.append("silent")
            elif abs(target_bid_utility_list[i + 1] - target_bid_utility_list[i]) == 0 and abs(
                    human_bid_utility_list[i + 1] - human_bid_utility_list[i]) > 0:
                target_move_list.append("nice")
            elif target_bid_utility_list[i + 1] - target_bid_utility_list[i] > 0 and human_bid_utility_list[i + 1] - \
                    human_bid_utility_list[i] > 0:
    ...
    def get_sensitivity_rate(self, target_move_list: t.List[str]):
        ...
        sensitivity_rate_dict = {"silent": 0.0, "nice": 0.0, "fortunate": 0.0, "unfortunate": 0.0, "concession": 0.0,
                                 "selfish": 0.0}
    ...
    def get_human_awareness(self, agent_preference_profile, human_preference_profile, agentHistory, humanHistory):
        ...
        human_moves_list = self.get_opponent_moves_list(human_preference_profile, agent_preference_profile,
                                                        humanHistory)
        agent_moves_list = self.get_opponent_moves_list(agent_preference_profile, human_preference_profile,
                                                        agentHistory)
    ...
    def get_sensitivity_index(self, target_preference_profile: UtilitySpace, opponent_preference_profile: UtilitySpace,
                              target_history: t.List[Offer]):
        """
        ...
        sensitivity_vector = np.array([[sensitivity_rate_dict["silent"], sensitivity_rate_dict["nice"],
                                        sensitivity_rate_dict["fortunate"], sensitivity_rate_dict["unfortunate"],
                                        sensitivity_rate_dict["concession"], sensitivity_rate_dict["selfish"]]])
