...
class HolidayOfferClassifier:
    ...
    def preprocess_offer_input(self, offer_input):
        ...
        for offer_verb in offer_verbs:
            for keyword in self.keywords:
                ...
                offer_input = re.sub(r'\b' + "(don't |do not )" + offer_verb + ".*" + keyword + r'\b', "", offer_input)
                offer_input = re.sub(r'\b' + 'instead of ' + keyword + r'\b', "", offer_input)
    ...
    def tag_offer_input_words(self, processed_offer_input):
        ...
        for word_index, (word, tag) in enumerate(tagged_words_list):
            ...
            if word.lower() in ["one", "two", "three", "1", "2", "3", "a"] and len(tagged_words_list) > (
                    word_index + 1):
    ...
    def check_if_acceptance(self, offer_input):
        offer_input = offer_input.lower()
        print("ACCEPTANCE CHECK: ", offer_input)
        # Check if the user accepts the offer.
        if (
                len(re.findall(r"\b" + "(no |not )deal" + r"\b", offer_input)) == 0
                and len(re.findall(r"\b" + "(deal|yes|agree|accept)" + r"\b", offer_input)) > 0
                and len(
            re.findall(
                r"\b"
