def get_single_move(self_utility, prev_self_utility, opponent_utility, prev_opponent_utility):
...
def get_target_move_list(target_utility_list, opponent_utility_list):
    """
...
def get_sensitivity_rate(target_move_list):
    """
...
def get_human_awareness(
    ...
    human_moves_list = get_target_move_list(
        human_offer_human_utilities, human_offer_agent_utilities
    )
    agent_moves_list = get_target_move_list(
        agent_offer_agent_utilities, agent_offer_human_utilities
    )

    # There is an empty element at the start of the moves list. We need that for logging purposes, it is not useful
    # here.
    ...
    count = 0
    # Calculate human's awareness by iterating and looking differences between offer utilities for both agent and
    # human. Since human is the first offer giver start from index 1.
...
def get_target_turn_move(
    ...
    if (
        ...
        return "unfortunate"
            ...
            target_utility - target_previous_utility < 0 < opponent_utility - opponent_previous_utility
    ):
        return "concession"
    else:
        return "selfish"
